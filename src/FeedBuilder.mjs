...
export default class FeedBuilder {
  ...
  /**
   * Adds an array of products to a feed.
   * @param {Array} products
   * @throws If argument is not array.
   */
  withProducts(products) {
    if (!Array.isArray(products)) {
      throw new Error('products expects an array of Product');
    }
  ...
  buildXml() {
    ...
    const xml = xmlbuilder.create({ encoding: 'utf-8' }, feed);
